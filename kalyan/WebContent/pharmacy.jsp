...
#table_details th {
 ...
 %>
    <body>
      <div>
        <div class="topnav">
            ...
            <a class="active" href="pharmacy.jsp">Pharmacy</a>
       ...
       // String id = resultset.getString(2);
         ...
         out.println("<div>");
            ...
            out.println("<center>");
            out.println("<h2>Issued Medicines For Patient</h2>");
   ...
   <br>
      <div>
       <center>
          <form method='POST' action = 'Pharmacy2'><input style = 'display: none' type='text' id='patid' name='patid'  value=${ pid }><button type='submit'name='issuemedicine' class='pure-button pure-button-primary' >Issue Medicines</button></form>
        </center>
      </div>
     ...
     <% } %>
          <% 
            ...
            ResultSet rs = 
            		st.executeQuery("select * from pharmacy") ; 
        %>
    <div>
        <center>
          ...
          <table id="table_details">
            ...
            <% while(rs.next()){ %>
                <tr>
                    ...
                    <td><%= rs.getInt(3) %></td>
                    <td><%= rs.getInt(4) %></td>
                    <td><form method="POST" action = "Pharmacy3">
                <input type="number" id="id" name="id" value=${ pid }			 readonly style="display: none;" >
                <input type="number" id="rate" name="rate" value = <%= rs.getInt(4) %> readonly style="display: none;" >
                <input type="number" id="quantity" name="quantity" value=<%= rs.getInt(3) %> readonly style="display: none;" >
                <button type="submit" id="medicinename" name="medicinename" style="width: 100%;" ${ disabled } value=<%= rs.getString(2) %>>Add</button>
              </form></td>
                </tr>
     		<% } %>
            </tbody>
        </table>
        <form method="POST" action = "pharmacy.jsp">
        <button type="submit" id="complete" name="complete" class="pure-button pure-button-primary">Completed</button>
        </form>
        </center>
    </div>
